...
function validateLogin() {
  ...
  if (!PORTAL_DATA) { showLoginError('Os dados do portal ainda não foram carregados.'); return; }
...
function renderPortal(data) {
  ...
  if (headerUpdate) headerUpdate.textContent = `${data.lastUpdate?.date || '--/--/----'} • ${data.lastUpdate?.time || '--:--'}`;
  if (subtitle) subtitle.textContent = data.appSubtitle || 'Consulta externa segura e visualização estratégica do relatório.';
...
function renderProducts(list) {
  ...
  el.innerHTML = list.map(p => `<div class="product-item"><div class="product-main"><strong>${escapeHtml(p.modelo || '-')}</strong><span>${escapeHtml(p.categoria || '-')}</span></div><div class="product-date">${escapeHtml(p.data || '-')}</div></div>`).join('');
}
...
function openReport() {
  if (!PORTAL_DATA?.report?.url) { showToast('Nenhum PDF configurado.'); return; }
...
function secretClick() {
  ...
  if (clickCount >= 5) { clickCount = 0; openAdminAuth(); }
}
...
function submitAdminAuth() {
  ...
  clearAdminAuthError();
  if (!password) { showAdminAuthError('Digite a senha administrativa.'); return; }
  if (password !== ADMIN_PASSWORD) { showAdminAuthError('Senha administrativa inválida.'); return; }
  closeAdminAuth();
  openAdmin();
}
...
function openAdmin() {
  if (!PORTAL_DATA) { showToast('Os dados do portal ainda não foram carregados.'); return; }
...
function loadPortal() {
  ...
  if (saved) {
    try { renderPortal(JSON.parse(saved)); return; }
    catch (e) { localStorage.removeItem('portalData'); }
  }
  fetch(CONFIG.API_URL).then(res => res.text()).then(text => renderPortal(JSON.parse(text))).catch(err => {
    console.error(err);
